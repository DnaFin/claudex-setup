 ...
 *
 * Cursor uses MDC format (.cursor/rules/*.mdc = YAML frontmatter + Markdown body),
 * and JSON for .cursor/mcp.json and .cursor/environment.json.
...
// ─── MDC parsing (YAML frontmatter + Markdown body) ─────────────────────────

/**
 * Parse an MDC file (YAML frontmatter delimited by --- + Markdown body).
...
/**
 * Minimal YAML parser for MDC frontmatter fields.
...
// ─── MDC rule type detection ─────────────────────────────────────────────────

/**
 * Determine the MDC rule type from frontmatter fields.
 *
 * Truth table:
 *   Always:           alwaysApply=true, globs=any, description=any
 *   Auto Attached:    alwaysApply=false, globs=set, description=optional
 *   Agent Requested:  alwaysApply=false, globs=empty, description=set
 *   Manual:           alwaysApply=false, globs=empty, description=empty
 */
function detectRuleType(frontmatter) {
  ...
  if (alwaysApply) return 'always';
  if (hasGlobs) return 'auto-attached';
  if (hasDescription) return 'agent-requested';
  return 'manual';
}
...
// ─── MDC frontmatter validation ──────────────────────────────────────────────

const VALID_MDC_FIELDS = new Set(['description', 'globs', 'alwaysApply']);

/**
 * Validate MDC rule file frontmatter.
...
function validateMdcFrontmatter(frontmatter) {
  ...
  for (const key of Object.keys(frontmatter)) {
    if (!VALID_MDC_FIELDS.has(key)) {
      errors.push(`Unknown MDC frontmatter field: "${key}"`);
    }
  }
  ...
  // Validate alwaysApply is boolean if present
  if (frontmatter.alwaysApply !== undefined && typeof frontmatter.alwaysApply !== 'boolean') {
    errors.push('alwaysApply must be a boolean (true or false)');
  }

  // Warn if globs + description combined (creates ambiguity)
  const hasGlobs = Array.isArray(frontmatter.globs)
    ? frontmatter.globs.length > 0
    : Boolean(frontmatter.globs);
  const hasDescription = Boolean(frontmatter.description && String(frontmatter.description).trim());
  if (hasGlobs && hasDescription && frontmatter.alwaysApply !== true) {
    errors.push('Combining globs + description creates ambiguity — use one or the other');
  }
...
/**
 * Validate MCP env vars use ${env:VAR} syntax (not hardcoded secrets).
 */
