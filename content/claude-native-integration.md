# Using nerviq from inside Claude Code
...
---
name: audit-repo
description: Run nerviq audit on the current project and show score + top gaps
---

Run `npx @nerviq/cli --json` on the current project directory.
...
```json
{
  "hooks": {
    "SessionStart": [
      {
        "type": "command",
        "command": "node -e \"try{const r=require('child_process').execSync('npx @nerviq/cli --json 2>/dev/null',{timeout:15000}).toString();const d=JSON.parse(r);if(d.score<50)console.log(JSON.stringify({systemMessage:'⚠️ Claude Code setup score: '+d.score+'/100. Consider running: npx @nerviq/cli --lite'}))}catch(e){console.log('{}')}\"",
        "timeout": 20,
...
1. Run `npx @nerviq/cli augment --json` on the current project
